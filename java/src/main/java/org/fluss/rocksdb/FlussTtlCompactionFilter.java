...
/**
 * Just a Java wrapper around FlussTtlCompactionFilter implemented in C++.
 *
 * Note: this compaction filter is a special implementation designed for use only by Fluss.
 */
public class FlussTtlCompactionFilter extends AbstractCompactionFilter<Slice> {
  ...
  public FlussTtlCompactionFilter(ConfigHolder configHolder, TimeProvider timeProvider) {
    this(configHolder, timeProvider, null);
  }

  public FlussTtlCompactionFilter(
      ConfigHolder configHolder, TimeProvider timeProvider, Logger logger) {
    super(createNewFlussTtlCompactionFilter0(
        configHolder.nativeHandle_, timeProvider, logger == null ? 0 : logger.nativeHandle_));
  }

  private native static long createNewFlussTtlCompactionFilter0(
      long configHolderHandle, TimeProvider timeProvider, long loggerHandle);
  private native static long createNewFlussTtlCompactionFilterConfigHolder();
  private native static void disposeFlussTtlCompactionFilterConfigHolder(long configHolderHandle);
  private native static boolean configureFlussTtlCompactionFilter(long configHolderHandle,
  ...
  public interface ListElementFilter {
     ...
     * for which element byte length is unknown and a custom type serializer has to be used
  ...
  private static class ConfigHolder extends RocksObject {
    ConfigHolder() {
      super(createNewFlussTtlCompactionFilterConfigHolder());
    }

    @Override
    protected void disposeInternal(long handle) {
      disposeFlussTtlCompactionFilterConfigHolder(handle);
    }
  }
  ...
  public static class FlussTtlCompactionFilterFactory
      extends AbstractCompactionFilterFactory<FlussTtlCompactionFilter> {
    private final ConfigHolder configHolder;
    private final TimeProvider timeProvider;
    private final Logger logger;

    @SuppressWarnings("unused")
    public FlussTtlCompactionFilterFactory(TimeProvider timeProvider) {
      this(timeProvider, null);
    }

    @SuppressWarnings("WeakerAccess")
    public FlussTtlCompactionFilterFactory(TimeProvider timeProvider, Logger logger) {
    ...
    @Override
    public FlussTtlCompactionFilter createCompactionFilter(Context context) {
      return new FlussTtlCompactionFilter(configHolder, timeProvider, logger);
    }

    @Override
    public String name() {
      return "FlussTtlCompactionFilterFactory";
    }

    @SuppressWarnings("WeakerAccess")
    public void configure(Config config) {
      boolean already_configured =
          !configureFlussTtlCompactionFilter(configHolder.nativeHandle_, config.stateType.ordinal(),
